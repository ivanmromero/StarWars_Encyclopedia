...
class VehiclesCategory: CategoryDataManage {
    ...
    func getSearchResultsCountFor(searchText: String?) -> Int {
        ...
        return vehicleResults.filter{$0.name.lowercased().contains(searchText.lowercased())}.count
    }
    
    func getNameOrTitleOfSearchResultAt(_ index: Int, searchText: String?) -> String? {
        guard let searchText = searchText else { return nil }
        let searchResults = vehicleResults.filter{$0.name.lowercased().contains(searchText.lowercased())}
        print(vehicleResults.filter{$0.name.lowercased().contains(searchText.lowercased())})
